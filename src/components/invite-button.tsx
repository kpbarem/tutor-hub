  ...
  return (
    <div>
      <button
        onClick={handleClick}
        disabled={loading}
        className="mt-5 w-full rounded-xl bg-white px-4 py-3 text-sm font-semibold text-primary-hover disabled:opacity-50"
      >
        {loading ? "Sending…" : "Send invitation"}
      </button>
      {status && (
        <p className={"mt-2 text-xs " + (status.success ? "text-white/80" : "text-red-200")}>
          {status.message}
        </p>
      )}
    </div>
